...
func (p *Parser) parseStatement() ast.Statement {
	var fn ast.Statement

	switch p.currentToken.Type {
	case token.LET:
		fn = p.parseIfStatement()
	case token.RETURN:
		fn = p.parseReturnStatement()
	default:
		fn = p.parseExpressionStatement()
	}
...
func (p *Parser) parseIfStatement() *ast.LetStatement {
...
func (p *Parser) parseBoolean() ast.Expression {
	value := "false"
	if strings.EqualFold(string(p.currentToken.Type), "true") {
		value = "true"
	}
	return &ast.Boolean{
		Token: p.currentToken,
